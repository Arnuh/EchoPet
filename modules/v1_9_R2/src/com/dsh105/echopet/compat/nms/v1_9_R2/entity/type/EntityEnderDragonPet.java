...
public class EntityEnderDragonPet extends EntityNoClipPet implements IComplex, IMonster, IEntityEnderDragonPet {
	...
	public void n(){
		...
		Entity passenger = this.bv().size() > 0 ? this.bv().get(0) : null;
		if(passenger != null && (passenger instanceof EntityHuman)){
            ...
            if (human.getBukkitEntity() == this.getPlayerOwner().getPlayer()) {
				// 1 after lastDamage
				float forw = ((EntityLiving) passenger).bf;// Middle one
				float side = ((EntityLiving) passenger).be;// Top one
				...
				this.aJ = this.aG = this.yaw;// After maxNoDamageTicks
        ...
        if (this.world.isClientSide) {
			...
			if(f1 <= -0.3F && f >= -0.3F && !this.ad()){
				this.world.a(this.locX, this.locY, this.locZ, getSoundFromString("entity.enderdragon.flap"), bA(), 5.0F, 0.8F + this.random.nextFloat() * 0.3F, false);
            }
        }
