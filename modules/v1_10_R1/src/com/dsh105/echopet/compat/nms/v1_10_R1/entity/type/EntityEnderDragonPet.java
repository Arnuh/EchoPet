...
public class EntityEnderDragonPet extends EntityNoClipPet implements IComplex, IMonster, IEntityEnderDragonPet{
	...
	public EntityEnderDragonPet(World world, IPet pet){
		...
		this.noClip(true);
		this.b = 100.0D;
		this.ai = true;// in EntityDragon constructor
	}
	...
	public void n(){
		...
		if(passenger != null && (passenger instanceof EntityHuman)){
			...
			if(human.getBukkitEntity() == this.getPlayerOwner().getPlayer()){
				// 1 after lastDamage
				float forw = ((EntityLiving) passenger).bg;// Middle one
				float side = ((EntityLiving) passenger).bf;// Top one
				Vector v = new Vector();
				...
				this.aK = this.aH = this.yaw;// aK is after maxNoDamageTicks aG is two above maxNoDamageTicks
