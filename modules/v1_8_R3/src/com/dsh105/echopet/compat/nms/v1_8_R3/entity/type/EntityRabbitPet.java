...
public class EntityRabbitPet extends EntityAgeablePet implements IEntityRabbitPet {
    ...
    public void onLive() {
		...
		if(this.onGround){
			...
			if(!jumpController.c()){
				if(this.delay == 0){
					PathEntity pathentity = ((PetGoalFollowOwner) petGoalSelector.getGoal("FollowOwner")).getNavigation().j();// Gets path towards the player.
	...
	public void E(){// Should we use m()? Idk difference.. M() is called on doTick in EntityInsentient
		super.E();
		if(this.delay > 0){
			this.delay -= 1;
		}
