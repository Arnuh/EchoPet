...
public abstract class EntityPet extends EntityCreature implements IAnimal, IEntityPet {
	...
	*/
	...
	public void g(float sideMot, float forwMot){// Look at EntityHorse for shit
		...
		if(FIELD_JUMP != null && passenger != null){
	...
	protected String getIdleSound(){
		return "mob." + pet.getPetType().getFixedSoundEffectEntityName() + ".ambient";
	}

	protected String getHurtSound(){
		return "mob." + pet.getPetType().getFixedSoundEffectEntityName() + ".hurt";
	}

	protected String getDeathSound(){
		return "mob." + pet.getPetType().getFixedSoundEffectEntityName() + ".death";
	}

	protected String getStepSound(){
		return "mob." + pet.getPetType().getFixedSoundEffectEntityName() + ".step";
	}
	...
	public void t_(){// Search for "entityBaseTick" the method calling the method its in uses it
		super.t_();
		if(!justCreated && !cf()){
			onLive();
			if(this.petGoalSelector == null){
			...
			if(!au() || getPet().getRider() == null) this.petGoalSelector.updateGoals();
        }
    }

	protected void h(){// Registers all the values into datawatcher
		super.h();
		initDatawatcher();
