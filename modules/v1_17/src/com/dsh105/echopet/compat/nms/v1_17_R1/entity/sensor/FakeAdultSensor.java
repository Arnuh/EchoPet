...
public class FakeAdultSensor extends AdultSensor{
	...
	private void setNearestVisibleAdult(AgeableMob var0, List<LivingEntity> var1){
		Optional<AgeableMob> var2 = var1.stream().filter((var1x)->{
			return var1x.getType() == var0.getType() && var1x instanceof AgeableMob;
		}).map(AgeableMob.class::cast).filter((var0x)->{
			return !var0x.isBaby();
		}).findFirst();
